...
function add(...args) {
    return Number((args.reduce((total, current) => total + current, 0)));
};

function subtract(num1, ...args) {
    return Number((args.reduce((total, current) => total - current, num1)));
};

function multiply(...args) {
    return Number((args.reduce((incrementer, next) => incrementer * next, 1)));
};

function divide(num1, ...args) {
    return Number((args.reduce((incrementer, next) => incrementer / next, num1)));
};
